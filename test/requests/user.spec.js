...
describe('# user request', () => {
  context('# tweets', () => {
    before(async () => {
      ...
      this.getUser = sinon.stub(helpers, 'getUser').returns({ id: 1, Followings: [], Replies: [], LikedTweets: [] })
  ...
  context('#likes', () => {
    before(async () => {
      this.ensureAuthenticated = sinon.stub(helpers, 'ensureAuthenticated').returns(true)
      this.getUser = sinon.stub(helpers, 'getUser').returns({ id: 1, Followings: [], LikedTweets: [], Replies: [] })
      await db.User.create({})
